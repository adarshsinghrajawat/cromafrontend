...
{ var product=props.product 
    ...
    const fetchDetails=async(id)=>{
        var result=await postData('userinterface/display_productdetails_by_id',{productdetailid:id})
    ...
    const showDetails=()=>{
        return details.map((item)=>{
          ...
          <div onClick={()=>handleChangeProduct(item?.productdetailid)} style={{ color: '#fff', fontSize: '12px', marginTop: '3%'}}>
